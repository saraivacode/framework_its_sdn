...
do
	...
	# 	#se o saldo for menor que zero
	 	if [[ $(cat saldo.txt | grep $i | cut -d' ' -f2) -lt 0 ]]; then
			#se for a rsu1 (que tem apenas como vizinha a rsu2)
			if [[ $i = "rsu1" ]]; then
				#Coleta os MACs na RSU1
				...
				while [[ $(cat saldo.txt | grep $i | cut -d' ' -f2) -lt 0 ]]; do
					...
					# verifica se rsu2 consegue ao menos suportar uma aplicação C
					if [[ $(cat saldo.txt | grep rsu2 | cut -d' ' -f2) -ge $appc_bw ]]; then
						...
						#verifica cada mac da RAN da RSU1 (origem), se esta com app C e redireciona enquanto tem saldo no vizinho
						while [[ $(cat saldo.txt | grep rsu2 | cut -d' ' -f2) -ge $appc_bw ]]; do
							#statements
							for i in $mac_rsu1;
							do
						...
						#se vizinho não suporta aplcação C, verifica se suporta B
					elif [[ $(cat saldo.txt | grep rsu2 | cut -d' ' -f2) -ge $appb_bw ]]
						echo -e "\n RSU1 continua congestionada. Verificando se é possivel redirecionar App B para RSU2..."

						while [[ $(cat saldo.txt | grep rsu2 | cut -d' ' -f2) -ge $appb_bw ]]; do
							#statements
							for i in $mac_rsu1;
							do
							...
							do
								...
								if [[ [[ $(cat saldo.txt | grep rsu1 | cut -d' ' -f2) -ge 0 ]] ]]; then
									...
									do
										...
										if [[ $res_b -ne "NULL" ]] && [[ $red_b -eq "NULL" ]]; then
