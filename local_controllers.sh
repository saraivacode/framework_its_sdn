...
do
	...
	do
		...
		#se a soma dos requisitos é maior que zero, calcula o saldo da RSU, armazena em $sd1 e imprime na tela resultados
		if [[ $y -gt 0 ]]; then
			sd1=$(echo $up-$y|bc)
			echo -e "\n " $(echo $j | cut -d'-' -f1) " tem ocupado " $(echo $y|bc) " e saldo de " $sd1 ". " $(echo $x| wc -w) " veiculos"
		fi
