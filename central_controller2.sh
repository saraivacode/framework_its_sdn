...
#Coleta os DPIDs dos switchs com respectivas interfaces para aplicar filas e salva em temp2.txt
mysql -u root -pwifi -e "select dpid,iface_updt from rsu where Region IN (select region from appkpi)" framework 2> /dev/null | grep '-' |
while read i;
do
	echo $i >> temp2.txt
done
